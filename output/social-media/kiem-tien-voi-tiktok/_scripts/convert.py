...
while i < len(lines):
    ...
    if '**[CHO NAY CHEN ANH:' in stripped or '**[CHỖ NÀY CHÈN ẢNH:' in stripped:
        p = doc.add_paragraph()
        run = p.add_run(stripped[2:].replace('**', '') if stripped.startswith('> ') else stripped.replace('**', ''))
    ...
    if img_match:
        ...
        if os.path.exists(path):
            try:
                ...
                doc.add_paragraph(f'[Khong tai duoc anh: {path} - {e}]')
        i += 1
        continue
