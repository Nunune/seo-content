...
python "$base\_scripts\preprocess.py" "$base\draft.md" "$base\preprocessed.md" "$base\images"
```
...
### Bước 6 — Kiểm tra output và báo cáo

Sau khi chạy xong, kiểm tra file đã tạo:
```powershell
$base = "D:\Nunu-Claude\seo_content\output\{category}\{slug}"
(Get-Item "$base\export.docx").Length
```

Dọn dẹp file tạm (chỉ xóa preprocessed — GIỮ LẠI scripts trong `_scripts/` để re-export):
```powershell
Remove-Item "$base\preprocessed.md" -Force -ErrorAction SilentlyContinue
```
...
Báo cáo ngắn gọn:
- Đường dẫn file: `seo_content/output/{category}/{slug}/export.docx`
- Kích thước file (bytes/KB)
