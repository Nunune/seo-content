...
while i < len(lines):
    ...
    if stripped.startswith('<!--'):
        # ASCII-only check: matches <!-- [ANH] or <!-- [ANH] or any variant of the image block
        is_image_block = bool(re.search(r'\[.*NH\]', stripped))
        block_lines = [line]
        if '-->' not in stripped:
            i += 1
        ...
        if is_image_block:
            ...
            file_match = re.search(r'File(?:\s+name)?:\s*(\S+\.(?:webp|png|jpg|jpeg))', block_text, re.IGNORECASE)
