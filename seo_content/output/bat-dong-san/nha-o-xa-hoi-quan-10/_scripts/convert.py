...
while i < len(lines):
    ...
    if img_match:
        ...
        if os.path.exists(path):
            try:
                ...
                doc.add_paragraph(f'[Khong tai duoc anh: {path}]')
        i += 1; continue
    ...
    if stripped.startswith('> - ') or stripped.startswith('> * '):
        ...
        bullet_run.font.name = FONT_NAME; bullet_run.font.size = Pt(10)
        parse_inline(p, text)
    ...
    if stripped.startswith('> '):
        ...
        parse_inline(p, text)
        for r in p.runs:
            r.font.size = Pt(10); r.font.color.rgb = RGBColor(0x4B, 0x55, 0x63)
        i += 1; continue
    ...
    if num_match:
        ...
        num_run.font.name = FONT_NAME; num_run.font.size = Pt(FONT_SIZE_BODY)
